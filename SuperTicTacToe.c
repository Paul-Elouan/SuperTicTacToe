...
char hardBotTurn(struct Grid superGrid[ROW][COLUMN], char letter, int *ptrBotRow, int *ptrBotCol)
{
    ...
    /* Finds cells taken by X and O */

    for(int gridRow = 0; gridRow < ROW; gridRow++)
    {
        for(int gridCol = 0; gridCol < COLUMN; gridCol++)
        {
            if(superGrid[superGridRow][superGridColumn].grid[gridRow][gridCol] == 'X')
            {
                XTakenCells[count] = gridRow*COLUMN + gridCol;
                count++;
            }
            else if(superGrid[superGridRow][superGridColumn].grid[gridRow][gridCol] == 'O')
            {
                OTakenCells[count] = gridRow*COLUMN + gridCol;
                count++;
            }
        }
    }
    ...
    for(int gridRow = 0; gridRow < ROW; gridRow++)
    {
        for(int gridCol = 0; gridCol < COLUMN; gridCol++)
        {
            if(superGrid[superGridRow][superGridColumn].grid[gridRow][gridCol] == 'O')
            {
                availableCells[count] = gridRow*COLUMN + gridCol;
                count++;
            }
